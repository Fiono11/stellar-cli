...
use std::{array::TryFromSliceError, fmt::Debug, fs::File, io::Write, num::ParseIntError};
...
impl Cmd {
    ...
    async fn run2(
        ...
        let tx = build_wrap_token_tx(
            &asset,
            &contract_id,
            0, // must come from threshold public key account
            self.fee.fee,
            network_passphrase,
            &key,
        )?;
        ...
        //println!("tx hash: {:?}", txn.hash(network_passphrase));

        let file_path: std::path::PathBuf =
            PathBuf::from_str("/Users/ruimorais/stellar-cli/cmd/soroban-cli/src/commands/contract")
                .unwrap();

        let mut tx_hash_file = File::create(file_path.join("tx_hash.json")).unwrap();

        let tx_hash_json =
            serde_json::to_string_pretty(&txn.hash(network_passphrase).unwrap()).unwrap();

        tx_hash_file.write_all(tx_hash_json.as_bytes()).unwrap();
